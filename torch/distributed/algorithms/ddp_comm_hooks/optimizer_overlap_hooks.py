...
def _apply_optim_in_backward_hook(
    ...
    optim_in_bwd_state = _OptimInBackwardHookState(
        optim_stream=_accelerator_context().Stream(),
    ...
    def apply_optim_in_backward_hook(
        ...
        with _accelerator_context().stream(optimizer_stream):
            fut.wait()
        ...
        def wait_for_optim_stream_callback():
            _accelerator_context().current_stream().wait_stream(optim_stream_state.optim_stream)
