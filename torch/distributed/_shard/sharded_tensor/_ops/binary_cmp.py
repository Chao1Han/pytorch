...
def _communicate_result(result, pg):
    ...
    if result:
        result_tensor = torch.ones(1, device=torch.device(torch.xpu.current_device()))
    else:
        result_tensor = torch.zeros(1, device=torch.device(torch.xpu.current_device()))

    dist.all_reduce(result_tensor, group=pg)

    expected_result = torch.ones(
        1, device=torch.device(torch.xpu.current_device())
