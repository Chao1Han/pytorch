...
class TestFullyShardOverlap(FSDPTest):
    ...
    One approach is to use xpu sleeps to emulate kernels in each stream;
    however, ``torch.xpu._sleep`` requires inputs in units of cycles. The
    ...
    def test_fully_shard_training_overlap(self):
        ...
        loss = model(inp).sum()  # warmup xpu and allocator
        loss.backward()
    ...
    def test_fully_shard_post_optim_event_overlap(self):
        ...
        run_train_steps(1, False)  # warmup xpu and allocator
        num_iters = 5
...
class Matmul(torch.autograd.Function):
    # Use xpu sleeps to emulate compute time
    @staticmethod
