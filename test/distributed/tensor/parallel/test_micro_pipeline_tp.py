...
class MicroPipelineTPTest(TestCase):
    ...
    def test_find_all_gather_patterns(self):
        ...
        inp = torch.rand(64, 32, device="xpu")
    ...
    def test_fuse_all_gather_scaled_matmul(self, A_dims, gather_dim):
        ...
        A_shard_shape[gather_dim] //= self.world_size
        A_shard = torch.rand(*A_shard_shape, device="xpu").to(torch.float8_e4m3fn)
        B = torch.rand(16, 32, device="xpu").to(torch.float8_e4m3fn).T
        ...
        if torch.xpu.get_device_capability() < (8, 9):
            return
    ...
    def test_fuse_scaled_matmul_reduce_scatter(self, A_dims, scatter_dim):
        ...
        if A_dims == 2:
            A = torch.rand(64, 32, device="xpu").to(torch.float8_e4m3fn)
        elif A_dims == 3:
            A = torch.rand(2, 64, 32, device="xpu").to(torch.float8_e4m3fn)
        else:
            raise AssertionError(f"Invalid A_dims: {A_dims}")
        B = torch.rand(16, 32, device="xpu").to(torch.float8_e4m3fn).T
        A_scale = torch.tensor(0.1, device="xpu")
    ...
    def test_dtensor_seq_par(self, shard_dim: int):
        model = MLPModule(device="xpu", bias=False)
        ...
        if shard_dim == 0:
            inp = torch.rand(8, 10, device="xpu")
        elif shard_dim == 1:
            inp = torch.rand(2, 8, 10, device="xpu")
        else:
