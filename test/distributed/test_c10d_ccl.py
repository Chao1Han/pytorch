...
if not c10d.is_available() or not c10d.is_ccl_available():
    print("c10d CCL not available, skipping tests", file=sys.stderr)
    sys.exit(0)
    ...
    def test_common_errors(self):
        ...
        with Env(without(vars, "WORLD_SIZE")):
            ...
            with self.assertRaisesRegex(ValueError, "WORLD_SIZE expected"):
                ...
                next(gen)
            c10d.init_process_group(backend="ccl", world_size=1)
        ...
        with Env(without(vars, "RANK")):
            ...
            with self.assertRaisesRegex(ValueError, "RANK expected"):
                gen = c10d.rendezvous("env://")
                next(gen)
            c10d.init_process_group(backend="ccl", rank=0)
            self.assertEqual(c10d.get_rank(), 0)
            self.assertEqual(c10d.get_world_size(), 1)
            c10d.destroy_process_group()
        ...
        with Env(withouts(vars, ["RANK", "WORLD_SIZE"])):
            self.assertEqual(None, os.environ.get("RANK"))
            self.assertEqual(None, os.environ.get("WORLD_SIZE"))
            c10d.init_process_group(backend="ccl", rank=0, world_size=1)
            self.assertEqual(c10d.get_rank(), 0)
            self.assertEqual(c10d.get_world_size(), 1)
            c10d.destroy_process_group()

        with Env(vars):
            c10d.init_process_group(backend="ccl")
            self.assertEqual(c10d.get_rank(), 0)
            self.assertEqual(c10d.get_world_size(), 1)
            c10d.destroy_process_group()
...
class TimeoutTest(test_c10d_common.AbstractTimeoutTest, TestCase):
    ...
    @retry_on_connect_failures
    @skip_but_pass_in_sandcastle_if(not TEST_XPU, "No GPUs available, skipping test")
    def test_default_store_timeout_ccl(self):
        self._test_default_store_timeout("ccl")


class ProcessGroupCCLGroupTest(MultiProcessTestCase):
    def _create_process_group_ccl(self, store, timeout=timedelta(seconds=600), device_id=None):
    ...
    def rank_to_GPU(self):
        ...
        return init_multigpu_helper(self.world_size, "ccl")


    @requires_ccl()
    @skip_but_pass_in_sandcastle_if(
        torch.xpu.device_count() < 2, "CCL test requires 2+ GPUs"
    )
    def test_close_multi_pg_unordered(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        pg = self._create_process_group_ccl(store)
    ...
    def test_file_store_check(self):
        ...
        dist.init_process_group(
            backend="ccl", rank=self.rank, world_size=self.world_size, store=store
        )
    ...
    @requires_ccl()
    @skip_but_pass_in_sandcastle_if(not TEST_MULTIXPU, "CCL test requires 2+ GPUs")
    def test_set_process_group_desc(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        device = torch.device(f"xpu:{self.rank}")
        pg_default = self._create_process_group_ccl(
...
class DistributedDataParallelTest(
    ...
    def _get_process_group(self):
        ...
        c10d.init_process_group(
            "ccl", store=store, rank=self.rank, world_size=self.world_size
        )
        return c10d.distributed_c10d._get_default_group()

    def _test_ccl_backend(
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_ddp_comm_hook_future_passing_gpu_ccl(self):
        """
        This unit test verifies whether the Future object is passed properly using ccl backend.
    ...
    def _test_ddp_comm_hook_allreduce_hook_ccl(
    ...
    def _test_default_ddp_comm_hooks_ccl(self, gradient_as_bucket_view=False):
        """
    ...
    def _test_powerSGD_ddp_comm_hook_ccl(self, gradient_as_bucket_view=False):
        """
    ...
    def _test_builtin_ddp_comm_hooks_ccl(self, gradient_as_bucket_view=False):
        """
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_ddp_comm_hook_allreduce_hook_ccl(self):
        self._test_ddp_comm_hook_allreduce_hook_ccl()

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_default_ddp_comm_hooks_ccl(self):
        self._test_default_ddp_comm_hooks_ccl()

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_fp16_compress_wrapper_ccl(self):
        self._test_fp16_compress_wrapper()

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_bf16_compress_wrapper_ccl(self):
        self._test_bf16_compress_wrapper()

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_builtin_ddp_comm_hooks_ccl(self):
        self._test_builtin_ddp_comm_hooks_ccl()

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_powerSGD_ddp_comm_hook_ccl(self):
        self._test_powerSGD_ddp_comm_hook_ccl()

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_ddp_comm_hook_allreduce_hook_ccl_grad_is_view(self):
        self._test_ddp_comm_hook_allreduce_hook_ccl(gradient_as_bucket_view=True)

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_ddp_comm_hook_allreduce_hook_ccl_static_graph(self):
        self._test_ddp_comm_hook_allreduce_hook_ccl(static_graph=True)

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_default_ddp_comm_hooks_ccl_is_view(self):
        self._test_default_ddp_comm_hooks_ccl(gradient_as_bucket_view=True)
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_builtin_ddp_comm_hooks_ccl_grad_is_view(self):
        self._test_builtin_ddp_comm_hooks_ccl(gradient_as_bucket_view=True)

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_powerSGD_ddp_comm_hook_ccl_grad_is_view(self):
        self._test_powerSGD_ddp_comm_hook_ccl(gradient_as_bucket_view=True)

    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_ddp_comm_hook_allreduce_with_then_hook_ccl(self):
        """
...
class CommTest(test_c10d_common.AbstractCommTest, MultiProcessTestCase):
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_broadcast_coalesced_ccl(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="ccl", store=store, rank=self.rank, world_size=self.world_size
        )
        process_group = c10d.distributed_c10d._get_default_group()
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_all_reduce_coalesced_ccl(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="ccl", store=store, rank=self.rank, world_size=self.world_size
        )
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_all_reduce_coalesced_manager_ccl(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="ccl", store=store, rank=self.rank, world_size=self.world_size
        )
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_ccl_barrier_device_ids(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="ccl", rank=self.rank, world_size=self.world_size, store=store
        )
    ...
    @requires_ccl()
    @skip_if_lt_x_gpu(2)
    def test_ccl_barrier_device_ids_function_argument(self):
        store = c10d.FileStore(self.file_name, self.world_size)
        c10d.init_process_group(
            backend="ccl", rank=self.rank, world_size=self.world_size, store=store
        )
...
class CclProcessGroupWithDispatchedCollectivesTests(
    test_c10d_common.ProcessGroupWithDispatchedCollectivesTests
):
    @requires_ccl()
    @skip_if_lt_x_gpu(1)
    def test_collectives(self):
        self._test_collectives(backend="ccl")

    @requires_ccl()
    @skip_if_lt_x_gpu(1)
    def test_allreduce_coalesced(self):
        self._test_allreduce_coalesced(backend="ccl")

    @requires_ccl()
    @skip_if_lt_x_gpu(1)
    def test_all_to_all_single(self):
        self._test_all_to_all_single(backend="ccl")
...
class LargeCommTest(test_c10d_common.AbstractLargeCommTest, MultiProcessTestCase):
    ...
    def test_new_group_local_sync(self):
        self._test_new_group_local_sync(backend="ccl")

    @requires_ccl()
    @skip_if_lt_x_gpu(4)
    def test_new_group_local_sync_sanity_check(self):
        self._test_new_group_local_sync_sanity_check(backend="ccl")

    @requires_ccl()
    @skip_if_lt_x_gpu(4)
    def test_new_group_local_sync_duplicated_pg(self):
        self._test_new_group_local_sync_duplicate_pg(backend="ccl")
    ...
    def _init_two_pg2_subgroups(self, world_size: int = 4):
        ...
        c10d.init_process_group(
            backend="ccl", store=store, rank=self.rank, world_size=world_size
        )
